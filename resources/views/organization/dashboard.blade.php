...
@section('page_subtitle', 'Selamat datang kembali, ' . session('organization_name') . '!')
...
<div class="space-y-10 animate-fade-in">
    ...
    <div class="bg-white rounded-[2rem] border border-slate-100 shadow-sm overflow-hidden">
        <div class="p-8 border-b border-slate-100 flex justify-between items-center">
            ...
            <a href="{{ route('organization.transactions.index') }}" class="text-indigo-600 font-bold hover:underline text-sm">Lihat Semua</a>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-50">
                    ...
                    <tr class="hover:bg-slate-50 transition">
                        ...
                        <td class="px-8 py-6">
                            <p class="font-bold uppercase tracking-wide text-sm truncate max-w-[150px] text-slate-800">{{ $trx->customer_name }}</p>
                            <p class="text-xs text-slate-400 truncate max-w-[150px]">{{ $trx->customer_email }}</p>
                        </td>
                        <td class="px-8 py-6 font-semibold text-slate-700 max-w-xs truncate">{{ $trx->event->title ?? '-' }}</td>
                        <td class="px-8 py-6 whitespace-nowrap">
                            @if($trx->status === 'settlement' || $trx->status === 'success')
                                <span class="px-3 py-1 bg-green-50 text-green-700 border border-green-100 rounded-lg text-xs font-bold uppercase">Success</span>
                            @elseif($trx->status === 'pending')
                                <span class="px-3 py-1 bg-orange-50 text-orange-700 border border-orange-100 rounded-lg text-xs font-bold uppercase">Pending</span>
                            @else
                                <span class="px-3 py-1 bg-rose-50 text-rose-700 border border-rose-100 rounded-lg text-xs font-bold uppercase">{{ $trx->status }}</span>
                            @endif
                        </td>
