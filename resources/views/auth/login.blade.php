...
<body class="bg-indigo-900 text-white min-h-screen flex items-center justify-center p-6">
    <div class="max-w-md w-full bg-white text-slate-900 rounded-[2rem] p-8 shadow-2xl">
        ...
        @if(session('error'))
            <div class="bg-red-100 text-red-600 p-4 rounded-xl mb-6 font-bold text-sm text-center">   {{ session('error') }}
            </div>
        @endif
...
@endif
        ...
        <div class="relative my-6 flex items-center justify-center">
            ...
            <span class="absolute bg-white px-3 text-xs text-slate-400 font-bold uppercase tracking-wider">atau</span>
        </div>

        <a href="{{ route('auth.google') }}" class="w-full py-4 border border-slate-200 rounded-2xl flex items-center justify-center gap-3 font-bold text-slate-700 hover:bg-slate-50 transition active:scale-95">
